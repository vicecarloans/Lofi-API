...
export class AlbumService {
  ...
  async getPublicAlbums(offset: number, limit: number): Promise<Album[]> {
    return await this.albumModel
      .find({ public: true })
      .populate({path: "tracks", select: "-__v -upload", populate: {path: "image", select: "-__v -upload"}})
  ...
  async getPublicPopularAlbums(offset: number, limit: number): Promise<Album[]> {
    return await this.albumModel
      .find({ public: true })
      .populate({
        path: "tracks",
  ...
  async _toggleVoteAlbum(albumId: string, action: VoteTypeEnum) {
    ...
    if (action === VoteTypeEnum.UPVOTE) {
      const score = popularityCalc(album.upvotes + 1, album.downvotes - 1);
  ...
  async upvoteAlbum(albumId: string, owner: string) {
    let shouldVote = true;
    // Find Vote Record
    const vote = await this.voteModel.findOne({ owner, subjectId: albumId });
